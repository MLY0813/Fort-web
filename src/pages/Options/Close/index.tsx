...
import { bigNumberToNormal, formatInputAddress, getBaseBigNumber, showEllipsisAddress } from "../../../libs/utils"
import './styles'
    ...
    useEffect(() => {
        ...
        if (selectToken) {
            ...
            ;(async () => {
                ...
                const balance:BigNumber = await selectTokenContract?.balanceOf(account)
                ...
                if (Boolean(tokenInfo[2])) {
                    fortAmount = (BigNumber.from(blockPrice).sub(BigNumber.from(tokenInfo[1]))).mul(balance).div(getBaseBigNumber(6))
                } else {
                    fortAmount = (BigNumber.from(tokenInfo[1])).sub(BigNumber.from(blockPrice)).mul(balance).div(getBaseBigNumber(6))
                }
                ...
                <div className={`${classPrefix}-rightCard-bottomInfo`}>
                    ...
                    <div className={`${classPrefix}-rightCard-bottomInfo-fortNum`}>
                        <div className={`${classPrefix}-rightCard-bottomInfo-fortNum-value`}>
                            {/* TODO:恢复正式代码 */}
                            {bigNumberToNormal(optionInfo?.fortAmount || BigNumber.from('0'), 18, 6)}
                    ...
                    <div className={`${classPrefix}-rightCard-bottomInfo-buttonDiv`}>
                        ...
                        {/* TODO:恢复正式代码 */}
                        <MainButton disable={closeButtonDis ? false : false} onClick={() => props.reviewCall(optionInfo!, false)}><Trans>Close</Trans></MainButton>
